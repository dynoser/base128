...
class vc128
{
    ...
    public static function init($encodeMode = 0) {
        ...
        $vcArr = \explode(' ', "Б в Г г Д д Ж ж з И и Й й к Л л м н П т У Ф ф Ц ц Ч ч Ш ш Щ щ Ъ ъ Ы ы ь Э э Ю ю Я я |");
        // не хватает ёЁ, заменяются б на 6, п на n, остальные кирилличные символы визуально идентичны латинице
        // символы в коде utf-8 (поэтому сам файл с php-кодом должен быть в utf-8). Последний символ однобайтовый.
        $cp1251 = \hex2bin('c1e2c3e3c4e4c6e6e7c8e8c9e9eacbebecedcff2d3d4f4d6f6d7f7d8f8d9f9dafadbfbfcddfddefedfff00');
        for(; $i < 128; $i++) {
            $p = $i - 85;
            $charEnc = $vcArr[$p];
